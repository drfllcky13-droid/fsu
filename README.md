...
- `sw.js`, `manifest.webmanifest`, `scenes.webmanifest` and the icons make both pages installable
  from a web address.
- `fsu-tests/` is the Playwright suite (`npm ci`, `npm run install-browser`, `npm test`; Playwright is pinned).
- `PLAN.md` holds the goals, rules, decisions, roadmap and open questions. `BRIEF.md` explains the
  structure and the things that bite. `HANDOFF.md` is the last session's report (see `CLAUDE.md`).
